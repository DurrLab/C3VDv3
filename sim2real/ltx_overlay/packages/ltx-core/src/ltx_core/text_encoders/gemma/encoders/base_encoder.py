...
# Changes: Added a _decoder_module() helper that resolves the underlying Gemma
# decoder across HF wrapper variants (model.model, language_model, get_decoder)
# so encode() works on both old and new transformers versions.
...
class GemmaTextEncoder(torch.nn.Module):
    ...
    def _decoder_module(self) -> torch.nn.Module:
        """Return the underlying Gemma text decoder across HF wrapper variants."""
        ...
        decoder = getattr(self.model, "language_model", None)
        if decoder is not None:
            return decoder
        ...
        raise AttributeError(f"Could not resolve decoder module for Gemma model type {type(self.model).__name__}")
    ...
    def encode(
        ...
        outputs = decoder(input_ids=input_ids, attention_mask=attention_mask, output_hidden_states=True)
