...
# Changes: Extended Gemma key remapping to also emit non-nested model.* keys and
# added a _resolve_submodule helper so create_and_populate works against both
# the nested (model.model.*) and direct (model.*) HF Gemma layouts.
...
GEMMA_LLM_KEY_OPS = (
    ...
    # 1. Map language model layers. Some HF versions expose these under
    # model.model.*, while others expose them directly under model.*.
    ...
    .with_kv_operation(
        operation=lambda key, value: [
            ...
            KeyValueOperationResult(key.replace("model.model.language_model.", "model.language_model.", 1), value),
        ],
        key_prefix="model.model.language_model.",
    )
    # 2. Map the Vision Tower, again supporting both nested and direct layouts.
    .with_matching(prefix="vision_tower.")
    .with_replacement("vision_tower.", "model.model.vision_tower.")
    .with_kv_operation(
        operation=lambda key, value: [
            KeyValueOperationResult(key, value),
            KeyValueOperationResult(key.replace("model.model.vision_tower.", "model.vision_tower.", 1), value),
        ],
        key_prefix="model.model.vision_tower.",
    )
    # 3. Map the Multi-Modal Projector for both layouts.
    ...
    .with_kv_operation(
        operation=lambda key, value: [
            ...
            KeyValueOperationResult(
                key.replace("model.model.multi_modal_projector.", "model.multi_modal_projector.", 1), value
            ),
        ],
        key_prefix="model.model.multi_modal_projector.",
    )
...
def _resolve_submodule(module: torch.nn.Module, *candidate_paths: str) -> torch.nn.Module:
    """Resolve the first available dotted submodule path."""
    for path in candidate_paths:
        ...
        try:
            ...
            continue
        return current
    raise AttributeError(f"Could not resolve any of the candidate paths: {candidate_paths}")


def create_and_populate(module: GemmaTextEncoder) -> GemmaTextEncoder:
    model = module.model
    v_model = _resolve_submodule(model, "model.vision_tower.vision_model", "vision_tower.vision_model")
    l_model = _resolve_submodule(
        model,
    ...
    if not hasattr(l_model, "embed_tokens"):
        l_model = _resolve_submodule(l_model, "model")
