# Modified from Lightricks/LTX-2 @ ae855f8.
# Changes: Added reference_strength and condition_image_strength controls so
# IC-LoRA reference frames and the I2V first-frame condition can be partially
# denoised at inference; added reference_resize_mode / condition_image_resize_mode
# (crop|stretch) to control how reference and condition frames are resized.
# Distributed under the LTX-2 Community License (see LICENSE).
...
class GenerationConfig:
    ...
    reference_resize_mode: Literal["crop", "stretch"] = "crop"  # Reference resize behavior before VAE encoding
    condition_image_resize_mode: Literal["crop", "stretch"] = "crop"  # Conditioning image resize behavior before VAE encoding
    ...
    # IC-LoRA reference conditioning strength in [0, 1]. 1.0 = full (reference kept clean,
    # current behavior), 0.0 = none (reference denoised along with target). Matches the
    # "attention_strength" / "video-conditioning strength" concept in the LTX docs.
    reference_strength: float = 1.0
    # First-frame image conditioning strength in [0, 1]. 1.0 = fully frozen (current
    # behavior), <1.0 = partial denoising of the condition tokens.
    condition_image_strength: float = 1.0
...
class ValidationSampler:
    ...
    def _generate_with_reference(self, config: GenerationConfig, device: torch.device) -> tuple[Tensor, Tensor | None]:
        ...
        ref_denoise_mask = torch.full(
            (1, ref_seq_len, 1),
            fill_value=1.0 - config.reference_strength,
            device=device,
            dtype=torch.float32,
        )
    ...
    def _apply_image_conditioning(
        ...
        encoded_image = self._encode_conditioning_image(
            image, config.height, config.width, device, config.condition_image_resize_mode
        )
        ...
        # Set denoise_mask for conditioned tokens from config.condition_image_strength.
        # 1.0 = fully frozen (denoise_mask=0, original behavior), <1.0 = partial denoising.
        new_denoise_mask = video_state.denoise_mask.clone()
        new_denoise_mask[:, :num_image_tokens] = 1.0 - config.condition_image_strength
    ...
    def _preprocess_reference_video(config: GenerationConfig) -> Tensor:
        ...
        if config.reference_resize_mode not in ("crop", "stretch"):
            raise ValueError(
                "reference_resize_mode must be 'crop' or 'stretch', "
                f"got {config.reference_resize_mode!r}"
            )

        # Resize reference frames to the model/reference target dimensions.
        if current_height != target_height or current_width != target_width:
            if config.reference_resize_mode == "stretch":
    ...
    def _validate_config(self, config: GenerationConfig) -> None:
        ...
        if not (0.0 <= config.reference_strength <= 1.0):
            raise ValueError(
                f"reference_strength must be in [0.0, 1.0], got {config.reference_strength}"
            )
        if not (0.0 <= config.condition_image_strength <= 1.0):
            raise ValueError(
                f"condition_image_strength must be in [0.0, 1.0], got {config.condition_image_strength}"
            )
    ...
    def _encode_conditioning_image(
        ...
        target_width: int,
        device: torch.device,
        resize_mode: Literal["crop", "stretch"] = "crop",
    ) -> Tensor:
        """Encode a conditioning image to latent space.
        resize_mode='crop'    -> resize-to-cover (aspect-preserving) + center crop.
        resize_mode='stretch' -> bilinear stretch directly to target (may distort aspect).
        """
        if resize_mode not in ("crop", "stretch"):
            raise ValueError(
                f"condition_image_resize_mode must be 'crop' or 'stretch', got {resize_mode!r}"
            )
